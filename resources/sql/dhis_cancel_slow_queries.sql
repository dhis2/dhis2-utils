...
declare
	...
	c integer := 0;
begin
	for q in select * from pg_catalog.pg_stat_activity where (now() - pg_stat_activity.query_start) > interval '2 minutes' and state != 'idle' and query not ilike '%pg_stat_activity%'
	loop
