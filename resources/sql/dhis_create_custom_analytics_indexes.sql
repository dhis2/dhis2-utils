--
-- SQL function for adding custom indexes to DHIS 2 analytics tables
--
-- Function can be run with a DHIS 2 analytics table hook or with a cron job
--
-- Index statements should be adjusted to match year and frequent and slow anaytics queries
--
-- Cron job to run every morning after analytics tables are populated and before business hours
--
-- 30 6 * * *       /bin/bash -c 'psql -d dhis2 -c "select dhis_create_analytics_indexes();"'
--

create function dhis_create_analytics_indexes()
returns void as $$
begin
    -- Partition 2025
