...
def export_metadata(metadata_type_selection: str):
    ...
    for metadata_type in metadata_type_user_selection:
        ...
        if not generate_template:
            ...
            # This is used to avoid the issue "API is trying to access or modify a row that is outside the
            # existing bounds of the sheet...
            rows_to_format_count = len(df) + 1
            resize_df = True
            if rows_to_format_count <= ws.row_count:
                rows_to_format_count = ws.row_count
                resize_df = False

            while True:
                try:
                    set_with_dataframe(worksheet=ws, dataframe=df, include_index=False,
                                       include_column_header=True, resize=resize_df)
                except gspread.exceptions.APIError as e:
                    time.sleep(30)
                    pass
                else:
        ...
        requests = apply_formatting_to_worksheet(ws, metadata_types_supported, current_worksheet_names, df_conf,
                                                 mandatory_fields[metadata_type], rows_to_format_count)
